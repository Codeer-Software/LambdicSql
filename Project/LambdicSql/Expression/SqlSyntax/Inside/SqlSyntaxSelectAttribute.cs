...
namespace LambdicSql.Expression.SqlSyntax.Inside
{
    class SqlSyntaxSelectAttribute : SqlSyntaxMethodAttribute
    {
        ...
        {
            //ALL, DISTINCT
            var modify = new List<System.Linq.Expressions.Expression>();
        ...
        static ExpressionElement ToStringSelectedElement(IExpressionConverter converter, ObjectCreateMemberInfo element)
        {
            //single select.
            //for example, COUNT(*).
            if (string.IsNullOrEmpty(element.Name)) return converter.Convert((object)element.Expression);

            //normal select.
            return LineSpace(converter.Convert((object)element.Expression), "AS", element.Name);
        }
